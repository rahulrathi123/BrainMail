...
gmail.observe.on("compose", function(compose,type) {
        var popup=null;
            $.ajax({
            type: "GET", 
            url: "https://smartmails-dot-raydio-test.appspot.com/popup/",
        ...
        gmail.tools.add_compose_button(compose, 'ANALYSE', function() {
        ...
        window.setTimeout( function() {
        var composeBody=''
        interval=setInterval(function(compose){

        var str1=compose.body();
        if(str1!=composeBody && str1)
        {
         composeBody=str1.slice(0);  

         $.post( "https://smartmails-dot-raydio-test.appspot.com/postmethod", { data: composeBody },function(resultdata,status,xhr){
...
{   
    ...
    if (scores.overall_score < 50) {
        ...
        $('#overall_heading').html("<strong>Your Overall Score </strong> <span style = 'color: #F75D59'>" + scores.overall_score +"</span>" +" / 100");

    } else {
        $('#overall_heading').html("<strong>Your Overall Score </strong> <span style = 'color: #4CC552'>" + scores.overall_score +"</span>" +" / 100");
    ...
    if (scores.complexity >= 50) {
        ...
        $('#complex_heading').html("<strong>Complexity </strong> <span style = 'color: #F75D59'>" + scores.complexity +"</span>" +" / 100");

    } else {
        $('#complex_heading').html("<strong>Complexity </strong> <span style = 'color: #4CC552'>" + scores.complexity +"</span>" +" / 100");

    }

    $('#word_count_heading').html("<strong>Length </strong>" + "  " + scores.word_count);


    var diff = 1 - (scores.politeness.polite + scores.politeness.rude);
    var polite_ratio = scores.politeness.polite / (scores.politeness.polite + scores.politeness.rude);
    var polite_add = polite_ratio * diff;

    var rude_ratio = 1 - polite_ratio;
    var rude_add = rude_ratio * diff;
    scores.politeness.polite += polite_add;
    scores.politeness.rude += rude_add;
...
var myChart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            label: 'Percentage',
            data: [scores.politeness.polite * 100, scores.politeness.rude*100],
            backgroundColor: [
                'rgba(76, 197, 82, 1)',
                'rgba(247, 93, 89, 1)',
            ],
    ...
    options: {
        //cutoutPercentage: 50, how much of a donught is it
        legend: {
            display: true,
            position: 'bottom',
            labels: {
    ...
    var love_score = Math.max(scores.tone.love - 0.167, 0);
    var joy_score = Math.max(scores.tone.joy - 0.167, 0);
    var surprise_score = Math.max(scores.tone.surprise - 0.167, 0);
    var fear_score = Math.max(scores.tone.fear - 0.167, 0);
    var anger_score = Math.max(scores.tone.anger - 0.167, 0);
    var sadness_score = Math.max(scores.tone.sadness - 0.167, 0);

    var total_score = love_score + joy_score + surprise_score + fear_score + anger_score + sadness_score;
    ...
    if (joy_score > 0) {
        chart_labels.push("Joy");
        chart_data.push((joy_score/total_score)*100);
        chart_colors.push("rgba(76, 197, 82, 1)");
    }

    if (love_score > 0) {
        chart_labels.push("Love");
        chart_data.push((love_score/total_score)*100);
        chart_colors.push("rgba(00, 32, 194, 1)");
    }

    if (surprise_score > 0) {
        chart_labels.push("Surprise");
        chart_data.push((surprise_score/total_score)*100);
        chart_colors.push("rgba(92, 179, 255, 1)");

    }

    if (fear_score > 0) {
        chart_labels.push("Fear");
        chart_data.push((fear_score/total_score)*100);
        chart_colors.push("rgba(224, 176, 255, 1)");

    }

    if (sadness_score > 0) {
        chart_labels.push("Sadness");
        chart_data.push((sadness_score/total_score)*100);
        chart_colors.push("rgba(255, 219, 88, 1)");

    }

    if (anger_score > 0) {
        chart_labels.push("Anger");
        chart_data.push((anger_score/total_score)*100);
        chart_colors.push("rgba(247, 93, 89, 1)");
    ...
    options: {
        //cutoutPercentage: 50, how much of a donought it is
         legend: {
            display: true,
            position: 'bottom',
            labels: {
